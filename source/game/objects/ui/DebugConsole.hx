...
class DebugConsole extends FlxSpriteGroup
{
	...
	public static function log(text:String, ?color:FlxColor)
	{
		...
		if (globalLog.length > 500) globalLog.shift();
		if (instance != null && instance.isVisible) instance.refreshDisplay();
	...
	public function toggle()
	{
		isVisible = !isVisible;
		visible = isVisible;
		if (isVisible) { scrollOffset = 0; refreshDisplay(); }
	}
	...
	function refreshDisplay()
	{
		...
		for (i in startIdx...endIdx)
			if (i >= 0 && i < globalLog.length) lines.push(globalLog[i].text);
		logText.text = lines.length == 0 ? "No messages yet. Script errors and traces will appear here." : lines.join("\n");
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (isVisible)
		{
			...
			if (FlxG.keys.justPressed.DELETE) { globalLog = []; refreshDisplay(); }
		}
	}

	override public function destroy()
	{
		if (consoleCamera != null) FlxG.cameras.remove(consoleCamera, true);
		instance = null;
		super.destroy();
	}
}
